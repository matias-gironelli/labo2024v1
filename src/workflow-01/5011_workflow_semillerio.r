...
envg$EXPENV <- list()
envg$EXPENV$exp_dir <- "~/buckets/b1/exp17/"
dir.create(envg$EXPENV$exp_dir, showWarnings = FALSE)
envg$EXPENV$wf_dir <- "~/buckets/b1/flow17/"
dir.create(envg$EXPENV$wf_dir, showWarnings = FALSE)
envg$EXPENV$wf_dir_local <- "~/flow17/"
...
DR_drifting_baseline <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$meta$script <- "/src/workflow-01/2_531_DR_corregir_drifting.r"
